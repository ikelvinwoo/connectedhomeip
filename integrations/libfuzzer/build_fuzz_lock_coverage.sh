...
# Builds linux lock-app to out/linux-x64-lock-no-ble-asan-libfuzzer-coverage-clang
# And collects coverage information. Saves to coverage_clusters.zip

set -e

FUZZ_DATE="$(date '+%Y%m%d_%H%M%S')"
CLUSTERS_DATE="lock_$FUZZ_DATE"
CHIP_ROOT="/home/kelvin/Documents/connectedhomeip"
COVERAGE_NAME="coverage_$CLUSTERS_DATE"
OUTPUT_ROOT="$CHIP_ROOT/out/linux-x64-lock-no-ble-asan-libfuzzer-coverage-clang"
...
./scripts/run_in_build_env.sh "./scripts/build/build_examples.py --target linux-x64-lock-no-ble-asan-libfuzzer-coverage-clang build"
...
set +e
sudo ASAN_OPTIONS=external_symbolizer_path=/home/kelvin/Documents/connectedhomeip/.environment/cipd/packages/pigweed/bin/llvm-symbolizer FUZZ_CAMPAIGN_MINUTES=$MINUTES ./out/linux-x64-lock-no-ble-asan-libfuzzer-coverage-clang/chip-lock-app-fuzzing $CORPUS $SEEDS $DICT_ARG $MAXLEN_ARG 1>/dev/null
set -e
